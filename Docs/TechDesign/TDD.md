...
| TDD-011 | `GET /api/scan?folder=&recursive=&force=` — `folder` required. Recursive unless `recursive` is `'false'` or `'0'`. Reuses `.vidorient/metadata.json` unless `force` or mtime/size changed. | PRD-002, PRD-015, PRD-027 |
...
| TDD-017 | `POST /api/scan/files` body `{ paths: string[], rootFolder?, force? }`. Probe with mtime cache; persist into each file’s directory `.vidorient/metadata.json`. `POST /api/scan/cancel` increments `scanGeneration` so in-flight probe chunks stop. | PRD-026, PRD-027 |
| TDD-018 | `GET /api/places` returns `{ home: os.homedir(), volumes: '/Volumes' }`. `GET /api/thumbnail?file=` serves a jpeg from that video’s directory `.vidorient/thumbs/` using `sendFile` with `dotfiles: 'allow'` (Express otherwise 404s hidden paths). `POST /api/thumbs/status` body `{ paths }` returns ready flags (notify-style; client already has listing state). Background thumb queue max 2. | PRD-027, PRD-028 |
...
| TDD-033 | Default browse path in UI: `/Volumes/Home Movies/Lori Movies`. Home uses `GET /api/places` `home` (`os.homedir()`). Pinned favorites: Home + `/Volumes`. Extra favorites persist in `localStorage` key `vidorient.favorites`. | PRD-001, PRD-028 |
